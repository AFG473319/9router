...
// Check if settings has AFRouter config
const hasAFRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["afrouter"];
};

// Read per-agent models.json and return current model id (without "afrouter/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["afrouter"]?.models;
...
// POST - Update AFRouter settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `afrouter/${model}`;

    // Remove all old afrouter/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("afrouter/"))
    ...
    // Add fresh afrouter models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`afrouter/${m}`] = {};
    });

    // Remove old afrouter model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("afrouter/")) {
    ...
    // Update models.providers.afrouter with all models
    settings.models.providers["afrouter"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `afrouter/${agentModel}` };
        return agent;
      });
...
export async function DELETE() {
  try {
    ...
    // Remove AFRouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["afrouter"];
    ...
    // Remove afrouter models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("afrouter/"));
    ...
    // Reset agents.defaults.model.primary if it uses afrouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("afrouter/")) {
