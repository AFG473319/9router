/**
 * `afrouter xai video` — generate a Grok Imagine video through the local
 * afrouter gateway and save the result as an MP4 file.
 *
...
const HELP = `
Usage: afrouter xai video --prompt "..." [options]

Generate a Grok Imagine video via your local afrouter gateway
...
Options:
  ...
  --api-key <key>         afrouter API key (or env AFROUTER_API_KEY)
...
function parseArgs(argv) {
  const opts = {
    ...
    port: DEFAULT_PORT,
    host: DEFAULT_HOST,
    apiKey: process.env.AFROUTER_API_KEY || null,
...
async function run(argv) {
  ...
  try {
    ...
    const connectionId = create.headers["x-afrouter-connection-id"] || null;
