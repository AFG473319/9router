...
// path, so x-afr-real-ip arrives straight from the client and a remote caller can claim to
...
describe("peer header trust", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    process.env.AFROUTER_PEER_TOKEN = PEER_TOKEN;
  ...
  afterEach(() => {
    ...
    delete process.env.AFROUTER_PEER_TOKEN;
  });
  ...
  it("rejects a spoofed loopback peer IP carrying a wrong trust token", async () => {
    const response = await proxy(request("/api/v1/models", {
      host: "172.18.192.1:20140",
      "x-afr-real-ip": "127.0.0.1",
      "x-afr-peer-token": "guessed-token",
    }));
...
describe("login limiter client IP", () => {
  ...
  it("buckets spoofed peer IPs together so lockout cannot be rotated away", () => {
    const first = getClientIp(request("/api/auth/login", { "x-afr-real-ip": "1.1.1.1" }));
    const second = getClientIp(request("/api/auth/login", { "x-afr-real-ip": "2.2.2.2" }));
  ...
  it("still honours TRUST_PROXY for operators fronting afrouter with a reverse proxy", () => {
