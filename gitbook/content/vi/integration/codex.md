...
Tích hợp AFRouter với OpenAI Codex CLI để định tuyến request API OpenAI qua hệ thống routing thông minh của AFRouter.
...
AFRouter cung cấp các model Codex sau:
...
1. Xác minh API key đúng trong AFRouter dashboard
...
1. Xác minh tên model khớp với cấu hình AFRouter
2. Kiểm tra kết nối provider OpenAI đang hoạt động trong AFRouter dashboard
...
Đảm bảo bạn đã cấu hình API key trong AFRouter cloud dashboard.
