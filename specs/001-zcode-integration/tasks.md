...
**Independent test**: Apply one model → parse `~/.zcode/v2/config.json`, confirm `9Router` entry (`kind: "openai-compatible"`, AFRouter baseURL, model key present); Reset → entry no longer routes to AFRouter, card leaves Connected.
