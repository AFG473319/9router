...
// Collect PIDs of all afrouter-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from afrouter install dir or wrapper cli.js
        const isAppProcess = lower.includes("afrouter") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("afrouter") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "afrouter");
  }
  return path.join(os.homedir(), ".afrouter");
}
...
// Resolve npx/afrouter binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx afrouter` — works regardless of global bin path changes after npm i -g
