...
export function applyOutboundProxyEnv(
  ...
  if (!enabled) {
    if (process.env.AFROUTER_PROXY_MANAGED === "1") {
      delete process.env.HTTP_PROXY;
      delete process.env.HTTPS_PROXY;
      delete process.env.ALL_PROXY;
      delete process.env.NO_PROXY;
      delete process.env.AFROUTER_PROXY_MANAGED;
      delete process.env.AFROUTER_PROXY_URL;
      delete process.env.AFROUTER_NO_PROXY;
    }
    return;
  }
  ...
  const wasManaged = process.env.AFROUTER_PROXY_MANAGED === "1";
  let managed = false;
  ...
  if (proxyUrl) {
    ...
    if (validated) {
      ...
      process.env.AFROUTER_PROXY_URL = validated;
      managed = true;
    }
  }
  ...
  if (noProxy) {
    ...
    process.env.AFROUTER_NO_PROXY = noProxy;
    managed = true;
  }

  if (managed) {
    process.env.AFROUTER_PROXY_MANAGED = "1";
  } else if (wasManaged) {
