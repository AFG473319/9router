...
const hasAFRouterConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["afrouter"];
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasAFRouter: hasAFRouterConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "AFRouter", baseURL: normalizedBaseUrl, apiKey };
