...
const hasAFRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "AFRouter");
};

const getAFRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "AFRouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getAFRouterEntry(config);
...
// POST - Apply AFRouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing AFRouter entry or append
    const idx = config.findIndex((e) => e.name === "AFRouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove AFRouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "AFRouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "AFRouter removed from Copilot config",
    });
  } catch (error) {
