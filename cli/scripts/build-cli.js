...
const cliAppDir = process.env.AFROUTER_CLI_APP_DIR || path.join(cliDir, "app");
...
function resolveStandaloneBuild(appDir, buildDistDir) {
  ...
  // NEXT_TRACING_ROOT_MODE=workspace, e.g. standalone/afrouter/server.js.
...
function buildCliPackage() {
  console.log("📦 Building AFRouter CLI package with Next.js...\n");
  ...
  // Strip better-sqlite3 (native) — it lives in ~/.afrouter/runtime to avoid
  ...
  if (fs.existsSync(betterDir)) {
    ...
    console.log("✅ Stripped better-sqlite3 (lives in ~/.afrouter/runtime)");
  }
  console.log("");
