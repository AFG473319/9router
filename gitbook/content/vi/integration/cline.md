...
Tích hợp AFRouter với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của AFRouter.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- AFRouter đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint AFRouter:
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng AFRouter để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua AFRouter
...
3. Nhận giải thích AI qua AFRouter

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng AFRouter để hiểu context và thực hiện thay đổi
...
1. Xác minh AFRouter đang chạy: `curl http://localhost:20128/health`
...
2. Xác minh AFRouter instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs AFRouter để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng AFRouter cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://9router.com`
2. Đảm bảo bạn đã cấu hình API key trong AFRouter cloud dashboard
...
## Tích hợp với Tính năng AFRouter

### Định tuyến Model

AFRouter tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, AFRouter tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua AFRouter dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
