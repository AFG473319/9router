...
Despliega AFRouter en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.afrouter` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/afrouter
sudo chown $USER:$USER /var/lib/afrouter
```
...
Crea `/etc/nginx/sites-available/afrouter`:
...
sudo ln -s /etc/nginx/sites-available/afrouter /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de AFRouter
...
tar -czf afrouter-backup-$(date +%Y%m%d).tar.gz /var/lib/afrouter

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/afrouter-$(date +\%Y\%m\%d).tar.gz /var/lib/afrouter
```
...
```bash
# Verificar si AFRouter está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/afrouter
chmod 755 /var/lib/afrouter
```
