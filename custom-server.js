...
// Per-process secret proving x-afr-real-ip was stamped below rather than sent by the client.
// A bare `next start` / `next dev` never loads this file, so it cannot produce a matching
// header even though the env var is inherited by child processes. Named like x-afr-cli-token
...
http.createServer = (...args) => {
  ...
  const wrapped = (req, res) => {
    ...
    delete req.headers["x-afr-real-ip"];
    delete req.headers["x-forwarded-for"];
    delete req.headers["x-afr-via-proxy"];
    delete req.headers["x-afr-peer-token"];
    req.headers["x-afr-real-ip"] = ip;
    req.headers["x-afr-peer-token"] = PEER_TOKEN;
    if (viaProxy) req.headers["x-afr-via-proxy"] = "1";
