# AFRouter Architecture

_Last updated: 2026-02-06_

## Executive Summary

AFRouter is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[AFRouter Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.afrouter/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.afrouter/usage.json`, `~/.afrouter/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.afrouter/db.json`)
- usage stats: `~/.afrouter/usage.json`
- request log lines: `~/.afrouter/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[AFRouter Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.afrouter` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/afrouter && npm run build`
- Build Docker image: `cd /root/dev/afrouter && docker build -t afrouter .`
