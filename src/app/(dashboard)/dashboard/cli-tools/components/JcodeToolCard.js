...
export default function JcodeToolCard({
  ...
  const currentBaseUrl = jcodeStatus?.config?.providers?.["afrouter"]?.base_url || "";

  const getConfigStatus = () => {
    if (!jcodeStatus?.installed) return null;
    if (!jcodeStatus?.hasAFRouter) return "not_configured";
    const currentProvider = jcodeStatus.config?.providers?.["afrouter"];
  ...
  useEffect(() => {
    if (jcodeStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = jcodeStatus.config?.providers?.["afrouter"];
      if (provider) {
...
[[providers.afrouter.models]]
id = "${selectedModel || "cc/claude-opus-4-7"}"`;

    const envContent = `JCODE_AFROUTER_API_KEY="${keyToUse}"`;
          ...
          {!checkingJcode && jcodeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {jcodeStatus?.config?.providers?.["afrouter"]?.base_url && (
                ...
                <div className="flex flex-col gap-1 p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg">
                  ...
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile afrouter</code>
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile afrouter --model {selectedModel || "cc/claude-opus-4-7"}</code>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!jcodeStatus?.hasAFRouter} loading={restoring}>
