...
// The dashboard is served by the router itself, so the browser origin is the
// authoritative base URL whatever port the server runs on.
...
export default function MitmServerCard({ apiKeys, cloudEnabled, onStatusChange }) {
  ...
  const [mitmRouterBaseUrl, setMitmRouterBaseUrl] = useState(getDefaultMitmRouterBase());
      ...
      if (action === "trust-cert") {
        ...
        res = await fetch("/api/cli-tools/antigravity-mitm", {
          ...
          body: JSON.stringify({
            ...
            mitmRouterBaseUrl: mitmRouterBaseUrl.trim() || getDefaultMitmRouterBase(),
            forceKillPort443,
          }),
        });
      } else {
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="px-2 py-2 rounded-lg bg-surface/50 border border-border/50 flex flex-col gap-2">
            <p className="text-[11px] text-text-muted leading-relaxed">
              <span className="font-medium text-text-main">Purpose:</span> Use Antigravity IDE & GitHub Copilot → with ANY provider/model from AFRouter
            </p>
            <p className="text-[11px] text-text-muted leading-relaxed">
              <span className="font-medium text-text-main">How it works:</span> Antigravity/Copilot IDE request → DNS redirect to localhost:443 → MITM proxy intercepts → AFRouter → response to Antigravity/Copilot
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2">
            <div className="grid gap-1 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
              <span className="text-xs font-semibold text-text-main sm:text-right sm:text-sm">AFRouter Base URL</span>
              <span className="material-symbols-outlined hidden text-text-muted text-[14px] sm:inline">arrow_forward</span>
              <input
                type="text"
                value={mitmRouterBaseUrl}
                onChange={(e) => setMitmRouterBaseUrl(e.target.value)}
                placeholder={getDefaultMitmRouterBase()}
                disabled={isRunning}
            ...
            {!isRunning && (
              <div className="grid gap-1 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                ...
                <input
                  ...
                  placeholder={cloudEnabled ? "Enter or pick API key" : "sk_afrouter (default)"}
          ...
          {serverIsWindows && !isAdmin && (
            <div className="flex items-center gap-2 px-2 py-1.5 rounded text-xs bg-red-500/10 text-red-600 border border-red-500/20">
              ...
              <span>Administrator required — restart AFRouter as Administrator to use MITM</span>
            </div>
          )}
        </div>
      </Card>
